#!/usr/bin/env python3
"""
10-row templating gel: build → honest FIRE → OpenMM → NVT MD → score → viewer.

  .venv/bin/python scripts/run_templating_10shell.py
  .venv/bin/python scripts/run_templating_10shell.py --md-ps 1
  .venv/bin/python scripts/run_templating_10shell.py --md-ns 0.5
"""
...
def main():
    ap = argparse.ArgumentParser(description="10-row templating gel + MD nucleation test")
    ...
    if not args.skip_fire:
        ...
        print(
            f"MD plan: {md_ns * 1000:.3f} ps ({md_ns:.6f} ns) at {args.md_temperature:.0f} K, "
